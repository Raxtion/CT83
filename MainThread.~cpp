...
void __fastcall CMainThread::Execute()
{
	...
	C_GetTime tmReset, tmAlarm, tmResetLamp, tmResetInitial;
	...
	while (1)
	{
		...
		//----Alarm Occured
		//--Stop Auto
		if (g_DIO.ReadDIBit(DI::StopBtn) || m_bSoftStop || (g_IniFile.m_nErrorCode>0 && g_IniFile.m_nErrorCode<1000))
		{
			m_bSoftStop = false;
        ...
        //---Reset Alarm
		if (g_DIO.ReadDIBit(DI::ResetBtn) || m_bSoftReset)
		{
		...
		else if (bHomeDone)			        //Manual Mode
		{
			...
			//Manual turn to Auto
			if (g_DIO.ReadDIBit(DI::StartBtn) || m_bSoftStart)
			{
				m_bSoftStart = false;
				bPreAuto = true;
				nThreadIndex[19] = 0;
...
void __fastcall CMainThread::doSprayerLane(int &nThreadIndex, bool bFront)
{
	...
	case 0:
		if (bFront)
		{
			if (m_bIsNeedPreMoveSprayer && !CheckXYZAtSprayReadyPos(bFront))
			{
				m_bIsNeedPreMoveSprayer = false;
				nThreadIndex++;
			}
	...
	case 10:
		...
		break;
	case 11:
	case nTagReadyWork:nThreadIndex = 11;
		if (p_tm1MS->timeUp())
		{
			//ShowNow
			m_arraybShape[0] = false;
	...
	case 16:
		...
		else if (!m_bStartSBTSpray)
		{
			m_bStartSBTSpray = true;          //restart  again
			CMainThread::nThreadIndex[17] = 0;
		}
		break;
	case 17:
	case nTagReadyOut:nThreadIndex = 17;
...
//---------------------------------------------------------------------------
void __fastcall CMainThread::doSBTSpray(int &nThreadIndex)
{
	static C_GetTime tm1MS(EX_SCALE::TIME_1MS, false);
...
void __fastcall CMainThread::doCleanSpray(int &nThreadIndex, bool bWater)
{
	...
	case nTagNextTimes:nThreadIndex = 5;
		...
		break;
	case 6:
		if (g_Motion.IsLastPosDone(Axis_Const::SPX))
		{
			g_Motion.RelMove(Axis_Const::SPX, -g_IniFile.m_dCleanSprayDistance);
			nThreadIndex++;
		}
